...
(defn password? [v _ _]
  (if (seq v)
    (not (nil? (re-matches #"(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9]).{8,}" (str v))))
    true))
...
(def default-validations
  {:not-empty             {:message "Please enter a value." :validator not-empty?}
   ...
   :valid-terminal        {:message "Value must contain 15 caracters"
                           :validator valid-terminal?}
   :password?             {:message "Password must contain at least one number and one uppercase and lowercase letter, and at least 8 or more characters"
                           :validator password?}
   ...
   :terms-and-conditions-checked?              {:message "You must confirm our Terms & Conditions."
                                                :validator checked?}})
